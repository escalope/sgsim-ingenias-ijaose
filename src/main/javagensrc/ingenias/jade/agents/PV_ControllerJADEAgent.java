...
public class PV_ControllerJADEAgent extends JADEAgent {

	public PV_ControllerJADEAgent() {
		super(new PV_ControllerProtocol(), new PV_ControllerInteractionLocks());
	}
	...
	private boolean initialiseNonConversationalTask(Task tobject) {
		...
		if (tobject.getType().equals("Enable_PV_Panels_only_if_needed")) {
			Vector<MentalEntity> expectedInput = null;
			RuntimeFact expectedOutput = null;
			RuntimeEvent expectedOutputEvent = null;
			RuntimeConversation expectedInt = null;
			...
			expectedInput = this.getMSM().getMentalEntityByType("_TimeTick4");
			if (expectedInput.size() == 0 && !("1".equals("0..n"))) {
				nonExistingInputs.add("_TimeTick4");
			} else {
				JADEAgent.addExpectedInputs(tobject, "_TimeTick4", "1",
						expectedInput);
			}
			allEntitiesExist = allEntitiesExist && expectedInput.size() != 0;
	...
	public Vector tasksThatSatisfyGoal(String goalname) {
		...
		if (goalname.equals("DoNotDumpExcessOfEnergy")) {

			{
				...
				// is tried
				tobject = new Enable_PV_Panels_only_if_neededTask(
	...
	public void setup() {
		...
		ttypes.add("Enable_PV_Panels_only_if_needed");
